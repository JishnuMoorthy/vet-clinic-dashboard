...
export default function PetsList() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Pets" actionLabel="Add Pet" onAction={() => navigate("/pets/new")} />
      <div className="relative max-w-sm">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input placeholder="Search pets..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
      </div>
      {filtered.length === 0 ? (
        ...
        <div className="rounded-md border">
          <Table>
            ...
            <TableBody>
              {filtered.map((pet) => (
                <TableRow key={pet.id} className="cursor-pointer" onClick={() => navigate(`/pets/${pet.id}`)}>
